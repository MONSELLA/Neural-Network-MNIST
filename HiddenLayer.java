...
public class HiddenLayer {
    ...
    public Double[] backwardPropagation(Double[] errorFromOutputLayer) {
        ...
        for (int i=0;i<numNeurons;i++) {
            ...
            for (int j=0;j<errorFromOutputLayer.length;j++) {
                // Compute the error contribution for the current neuron
                neuronError += errorFromOutputLayer[j]*weights[j];
            }
            error[i]=neuronError;
    ...
    public Double[] updateWeights(double learningRate, Double[] hiddenLayerErrors,Double[] input){
        ...
        //return the outputs from the hidden layer to the output layer
        return outputs;
    }
}
